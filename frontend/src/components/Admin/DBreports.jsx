...
export default function DBreports() {
  ...
  const COLORS = ["#3B82F6", "#F59E0B"];
    ...
    useEffect(() => {
        ...
        const fetchListingsPatched = async () => {
            try {
                ...
                setApprovedListings(response.data.filter(listing => listing.status === 'approved').length);
                ...
                response.data.forEach(listing => {
                    const month = new Date(listing.createdAt).toLocaleString('en-us', { month: 'short' });
                    monthlyListings[month] = (monthlyListings[month] || 0) + 1;
                    const type = listing.homeType || 'Other';
                    propertyTypes[type] = (propertyTypes[type] || 0) + 1;
                });
                const sortedMonths = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
                const processedLineChartData = sortedMonths
    ...
    const filteredApprovedListings = filteredListings.filter(l => l.status === 'approved').length;
    const filteredPendingListings = filteredListings.filter(l => l.status === 'pending').length;
    const filteredTotalTransactions = filteredTransactions.reduce((sum, t) => sum + (t.amount || 0), 0);
    ...
    <div className="space-y-6 w-full">
        ...
        <div className="flex justify-end mt-4 mr-6">
            <div className="flex gap-2">
                <button onClick={() => setFilterType('Today')} className={`px-4 py-2 ${filterType === 'Today' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 border border-gray-200'} rounded-md hover:bg-gray-50 transition-colors`}>Today</button>
                <button onClick={() => setFilterType('Monthly')} className={`px-4 py-2 ${filterType === 'Monthly' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 border border-gray-200'} rounded-md hover:bg-gray-50 transition-colors`}>Monthly</button>
                <button onClick={() => setFilterType('Yearly')} className={`px-4 py-2 ${filterType === 'Yearly' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 border border-gray-200'} rounded-md hover:bg-gray-50 transition-colors`}>Yearly</button>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {[{ label: "Total Listings", value: filteredListings.length }, { label: "Approved Listings", value: filteredApprovedListings }, { label: "Pending Listings", value: filteredPendingListings }, { label: "Total Users", value: filteredUsers.length }, { label: "Total Transactions", value: `Rs. ${filteredTotalTransactions.toLocaleString()}` },].map((card, idx) => (
            <div
              key={idx}
